/datum/species/can_equip(obj/item/I, slot, disable_warning, mob/living/carbon/human/H, bypass_equip_delay_self, ignore_equipped, indirect_action)
	...
	switch(slot)
		...
		//hsector
		if(ITEM_SLOT_PENIS)
			var/obj/item/organ/external/genital/G = H.get_organ_slot(ORGAN_SLOT_PENIS)
			if(G)
				return equip_delay_self_check(I, H, bypass_equip_delay_self)
			if(!disable_warning)
			...
			return FALSE
		if(ITEM_SLOT_VAGINA)
			var/obj/item/organ/external/genital/G = H.get_organ_slot(ORGAN_SLOT_VAGINA)
			if(G)
				return equip_delay_self_check(I, H, bypass_equip_delay_self)
			if(!disable_warning)
			...
			return FALSE
		if(ITEM_SLOT_ANUS)
			var/obj/item/organ/external/genital/G = H.get_organ_slot(ORGAN_SLOT_ANUS)
			if(G)
				return equip_delay_self_check(I, H, bypass_equip_delay_self)
			if(!disable_warning)
