...
/mob/dead/new_player/update_title_screen()
	. = ..()
	if(client?.prefs?.read_preference(/datum/preference/toggle/hide_splashscreen) && SStitle.current_title_screen != DEFAULT_TITLE_SCREEN_IMAGE && SStitle.current_title_screen != DEFAULT_TITLE_LOADING_SCREEN)
