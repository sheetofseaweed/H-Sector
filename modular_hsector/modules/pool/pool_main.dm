...
/*/obj/effect/overlay/water //in modular_skyrat\modules\mapping\code\pool.dm
	name = "water"
...
/*/turf/open/mouse_drop_receive(atom/from, mob/living/user)
	to_chat(world, "mouse_drop_receive")
	if(!istype(user))
		return ..()
	to_chat(world, "if(!istype(user))")
	// I could make this /open/floor and not have the !istype but ehh - kev

	if(HAS_TRAIT(from, TRAIT_SWIMMING))
		to_chat(world, "HAS_TRAIT(from, TRAIT_SWIMMING)")
	if(((user == from) || user.CanReach(from)))
		to_chat(world, "(user == from) || user.CanReach(from))")
	if(!istype(src, /turf/open/pool))
		to_chat(world, "!istype(src, /turf/open/pool)")
	if((user.mobility_flags & MOBILITY_USE))
		to_chat(world, "(user.mobility_flags & MOBILITY_USE)")

	if(HAS_TRAIT(from, TRAIT_SWIMMING) && isliving(user) && ((user == from) || user.CanReach(from)) && (user.mobility_flags & MOBILITY_USE) && !istype(src, /turf/open/pool))
...
/turf/open/pool/Exit(atom/movable/AM, direction)
	...
	if(isliving(AM) || isstructure(AM))
		...
		if(isliving(AM) && (locate(/obj/structure/pool/ladder) in src))
			return ..()			//climbing out
		var/turf/new_turf = get_step(AM, direction)
		if(istype(new_turf, /turf/open/pool))
			return ..()
		return FALSE
	return ..()
...
/turf/open/pool/mouse_drop_receive(mob/living/dropped_mob, mob/living/user, params)
	//. = ..()
	if(!isliving(dropped_mob))
		return
	var/mob/living/victim = dropped_mob
	if(user.stat || (user.body_position == LYING_DOWN) || !Adjacent(user) || !dropped_mob.Adjacent(user) || !iscarbon(user) || !victim.has_gravity(src) || HAS_TRAIT(victim, TRAIT_SWIMMING))
		return
...
/turf/open/pool/attack_hand(mob/living/user, unarmed_attack_flags)
	. = ..()
	if(.)
		return
	if((user.loc != src) && !user.IsStun() && !user.IsKnockdown() && !user.incapacitated && Adjacent(user) && HAS_TRAIT(user, TRAIT_SWIMMING) && filled && (next_splash < world.time))
		playsound(src, 'modular_hsector/sound/effects/watersplash.ogg', 8, TRUE, 1)
