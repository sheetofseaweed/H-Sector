...
/obj/structure/pool/ladder/attack_hand(mob/living/user, unarmed_attack_flags)
	. = ..()
	if(.)
		return
...
/obj/structure/pool/Lboard/attack_hand(mob/living/user, unarmed_attack_flags)
	if(iscarbon(user))
...
/obj/structure/pool/Lboard/proc/dive(mob/living/carbon/jumper, original_layer, original_px, original_py)
	switch(rand(1, 100))
		...
		if(91 to 100)
			jumper.visible_message("<span class='notice'>[jumper] is preparing for the legendary dive! Can [jumper.p_their()] make it?</span>", \
