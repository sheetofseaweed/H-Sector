...
/obj/machinery/pool/drain/process()
	...
	if(active)
		if(filling)
			if(cycles_left-- > 0)
				playsound(src, 'modular_hsector/sound/effects/fillingwatter.ogg', 100, TRUE)
			...
			if(cycles_left-- > 0)
				playsound(src, 'modular_hsector/sound/effects/pooldrain.ogg', 100, TRUE)
				...
				for(var/mob/M in orange(min(drain_suck_range, 10), src))
					if(isliving(M) && !M.anchored && is_in_our_pool(M))
						...
						if(ishuman(M) && (get_dist(M, src) <= 1))
							...
							playsound(src, pick('sound/effects/wounds/crack1.ogg','sound/effects/wounds/crack2.ogg'), 50, TRUE)
...
/obj/machinery/pool/filter/proc/spawn_shark()
	new /obj/item/toy/plush/shark(loc)
	/*if(prob(50))
...
/obj/machinery/pool/filter/attack_hand(mob/user, unarmed_attack_flags)
