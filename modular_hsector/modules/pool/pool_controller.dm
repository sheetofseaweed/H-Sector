...
/obj/machinery/pool
	icon = 'modular_hsector/icons/obj/machines/pool.dmi'
	anchored = TRUE
...
/obj/machinery/pool/controller/ShiftClick(mob/living/user)
	. = ..()
	if(!istype(user))
		return FALSE
	if(!user.Adjacent(src) || !user.CanReach(src) || !(user.mobility_flags & MOBILITY_USE))
		return FALSE
		...
		if(W.reagents.total_volume) //check if there's reagent
			...
			if(do_after(user, 50, target = src))
				...
				for(var/datum/reagent/R in reagents.reagent_list)
					if((R.volume >= min_reagent_amount) && (!respect_reagent_blacklist || (R.chemical_flags & REAGENT_CAN_BE_SYNTHESIZED)))
...
/obj/machinery/pool/controller/proc/process_pool()
	...
	for(var/mob/living/M in mobs_in_pool)
		switch(temperature) //Apply different effects based on what the temperature is set to.
			...
			if(POOL_WARM) //Warm
				M.adjust_bodytemperature(20,0,274.15) //Heats up mobs till the thermometer shows up
			//Normal temp does nothing, because it's just room temperature water.
			if(POOL_COOL)
				M.adjust_bodytemperature(-20,323.15) //Cools mobs till the thermometer shows up
