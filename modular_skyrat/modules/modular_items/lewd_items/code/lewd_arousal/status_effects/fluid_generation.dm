...
/datum/status_effect/body_fluid_regen/vagina/tick(seconds_between_ticks)
	...
	if(owner.stat >= DEAD || !istype(affected_human)) //	if(owner.stat >= DEAD || !owner.client?.prefs?.read_preference(/datum/preference/toggle/erp) || !istype(affected_human))
		return FALSE
...
/datum/status_effect/body_fluid_regen/testes/tick(seconds_between_ticks)
	var/mob/living/carbon/human/affected_human = owner
	if(owner.stat >= DEAD || !istype(affected_human)) //if(owner.stat >= DEAD || !owner.client?.prefs?.read_preference(/datum/preference/toggle/erp/sex_toy) || !istype(affected_human))
		return FALSE
