...
/mob/living/carbon/human/get_clothing_examine_info(mob/living/user)
	...
	if(crotch && !crotch_hidden() && !(crotch.item_flags & EXAMINE_SKIP))
		. += "[t_He] [t_is] wearing [crotch.examine_title(user)] on [t_his] crotch."
	//p
	if(penis && !penis_hidden() && !(penis.item_flags & EXAMINE_SKIP))
		. += "[t_He] [t_is] wearing [penis.examine_title(user)] on [t_his] penis."
	//v
	if(vagina && !vagina_hidden() && !(vagina.item_flags & EXAMINE_SKIP))
		. += "[t_He] [t_has] a [vagina.examine_title(user)] in [t_his] vagina."
	//a
	if(anus && !anus_hidden() && !(anus.item_flags & EXAMINE_SKIP))
		. += "[t_He] [t_has] a [anus.examine_title(user)] in [t_his] anus."
	//n
	if(nipples && !nipples_hidden() && !(nipples.item_flags & EXAMINE_SKIP))
		. += "[t_He] [t_is] wearing [nipples.examine_title(user)] on [t_his] nipples."
	//m
	if(mouth && !mouth_hidden() && !(mouth.item_flags & EXAMINE_SKIP))
		. += "[t_He] [t_has] a [mouth.examine_title(user)] in [t_his] mouth."
